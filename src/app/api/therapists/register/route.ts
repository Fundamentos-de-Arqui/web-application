    ...
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: `Error al registrar terapeuta: ${response.statusText}. ${errorText}` },
        ...
        return NextResponse.json({ success: true, message: 'Terapeuta registrado exitosamente' });

    } catch (error) {
        console.error('Error in therapist registration proxy:', error);
        return NextResponse.json(
            { error: error instanceof Error ? error.message : 'Error desconocido al registrar terapeuta' },
            { status: 500 }
        );
    }
}